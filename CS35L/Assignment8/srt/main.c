...
/* Declare global variables used in draw and main */
...
static void* draw(void* arg)
{
    ...
    argStruct* argument = (argStruct*) arg;
    int id = argument->id;
...
int main( int argc, char **argv )
{
    ...
    colors = (float*) malloc(sizeof(float) * size);
    pthread_t*  threadID = (pthread_t*) malloc(sizeof(pthread_t) * nthreads);
    argStruct* argument = (argStruct*) malloc(sizeof(argStruct) * nthreads);
    
    if (threadID == NULL || argument == NULL || colors == NULL)
    {
    ...
    for (int i = 0; i < nthreads; i++)
    {
        if (pthread_create(&threadID[i], NULL, draw, (void*) &argument[i]))
        {
